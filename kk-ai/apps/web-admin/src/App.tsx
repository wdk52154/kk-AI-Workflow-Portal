...
export default function App({ theme, resolvedTheme, setTheme, toggleTheme }: AppProps) {
  ...
  const themeIcon = resolvedTheme === 'dark' ? <SunOutlined /> : <MoonOutlined />;
  ...
  return (
    <ProLayout
      ...
      menuItemRender={(item, dom) => (
        <div onClick={() => item.path === '/' || message.info(`${item.name} 模块开发中`)}>
          {dom}
        </div>
      )}
      actionsRender={() => [
    ...
    >
      {/* ─── 统计卡片 ─── */}
      <StatisticCard.Group direction="row" gutter={[16, 16]} style={{ marginBottom: 24 }}>
        <StatisticCard
          statistic={{
            title: '今日调用',
            value: '8,432',
            description: <span style={{ color: '#52c41a' }}>↑ 12.5% 较昨日</span>,
        ...
        <StatisticCard
          statistic={{
            ...
            icon: <ThunderboltOutlined style={{ color: '#f59e0b', fontSize: 24 }} />,
          }}
          style={{ flex: 1 }}
        />
        <StatisticCard
          statistic={{
            ...
            icon: <SafetyOutlined style={{ color: '#ef4444', fontSize: 24 }} />,
          }}
          style={{ flex: 1 }}
        />
      </StatisticCard.Group>
      ...
      >
        ...
        >
          <Space wrap>
            ...
            <Button icon={<DesktopOutlined />} onClick={() => setTheme('system')}>
              系统
            </Button>
            <Button type="primary" icon={<ThunderboltOutlined />} onClick={toggleTheme}>
              切换
            </Button>
          </Space>
        </ProCard>
        ...
        >
          <Space direction="vertical" size={8} style={{ width: '100%' }}>
            ...
            >
              <span style={{ color: '#8c8c8c' }}>项目名称</span>
              <span style={{ fontWeight: 600 }}>{demoProject.name}</span>
            </div>
            <div
              style={{
                display: 'flex',
            ...
            >
              <span style={{ color: '#8c8c8c' }}>日配额</span>
            ...
            >
              <span style={{ color: '#8c8c8c' }}>月配额</span>
              <span style={{ fontWeight: 600, fontFamily: 'monospace' }}>
        ...
        >
          <Space direction="vertical" size={8} style={{ width: '100%' }}>
            {[
              { icon: <CloudServerOutlined />, text: 'pnpm workspace + Turbo 构建编排' },
              { icon: <CodeOutlined />, text: 'Vite + React 18 + TypeScript Strict' },
              { icon: <ThunderboltOutlined />, text: 'Ant Design + ProComponents 主题系统' },
              { icon: <ApiOutlined />, text: 'FastAPI Gateway (mcp-hub:8000)' },
              { icon: <DatabaseOutlined />, text: '@kk-ai/ui / utils / types 共享包' },
            ].map((item) => (
              <div
                key={item.text}
                style={{
